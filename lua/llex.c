        ...
        switch (ls->current) {
          ...
          case 'x': /* \xFF */
          {
            ...
            if (!isxdigit(ls->current)) {
              luaX_lexerror(ls, "expected hex digit", TK_STRING);
            }
            c = ls->current <= '9' ? 
          ...
          /* \u0000 .. \u10ffff unicode chars map to utf-8 sequence */
          case 'u':
          {
            int i;
            ...
            /* now encode the value as a sequence of bytes, UTF-8 encoded */
            if (c <= 0x7f) {
              save(ls, c);
            } else if (c <= 0x7ff) {
              ...
              save(ls, 0xe0 | ((c >> 12) & 0xf));
              save(ls, 0x80 | ((c >> 6) & 0x3f));
              save(ls, 0x80 | (c & 0x3f));
            } else if (c <= 0x1fffff) {
              save(ls, 0xf0 | ((c >> 18) & 0x7));
              save(ls, 0x80 | ((c >> 12) & 0x3f));
              save(ls, 0x80 | ((c >> 6) & 0x3f));
              save(ls, 0x80 | (c & 0x3f));
            } else if (c <= 0x3ffffff) {
              save(ls, 0xf8 | ((c >> 24) & 0x3));
              save(ls, 0x80 | ((c >> 18) & 0x3f));
              save(ls, 0x80 | ((c >> 12) & 0x3f));
              save(ls, 0x80 | ((c >> 6) & 0x3f));
              save(ls, 0x80 | (c & 0x3f));
            } else if (c <= 0x7fffffff) {
              save(ls, 0xfc | ((c >> 30) & 0x1));
              ...
              luaX_lexerror(ls, "unicode escape sequence too large",
                TK_STRING);
            }
            continue;
          }
