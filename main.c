...
static void catch_other(int sig_num)
{
  ...
  fprintf(stderr, "gimli-monitor: caught signal %d, terminating.\n", sig_num);
  should_exit = 1;
  respawn = 0;
  for (p = procs; p; p = p->next) {
...
static int prep(void)
{
  ...
  if (fd == -1) {
    fprintf(stderr, 
      "gimli-monitor: failed to open heartbeat file: %s\n", hb_file);
    hb_file[0] = '\0';
    return 0;
  }
  if (debug) {
  ...
  if (debug) {
    fprintf(stderr, "gimli-monitor: mmap fd=%d -> addr %p (%s)\n",
      fd, addr, strerror(errno));
  }
  close(fd);

  if (addr == MAP_FAILED) {
    perror("gimli-monitor: failed to map heartbeat memory");
    return 0;
  }
  ...
  snprintf(var, sizeof(var), "GIMLI_HB_ID=%s", hb_file);
  putenv(strdup(var));
...
static void trace_child(struct kid_proc *p)
{
  ...
  snprintf(cmdbuf, sizeof(cmdbuf)-1, "%s", "./gimli_coroner");
